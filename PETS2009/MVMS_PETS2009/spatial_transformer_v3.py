...
class SpatialTransformer_2DTo2D_real(Layer):
    ...
    def call(self, inputs, mask=None):
        ...
        output = tf.compat.v1.placeholder(dtype=tf.float32)

        #for i in range(len(view)):
        view_i = view
        scale = self.scale
        self.proj_2DTo2D(view_i, inputs[0:1, ...],scale)
        output_i = self.proj_splat(inputs[0:1, ...]) #inputs
    ...
    def proj_2DTo2D(self, view, inputs, scale):
        w = 768
        h = 576
        if (scale==1):
            W = 610
        ...
        if(scale==1):
            grid_rangeX = grid_rangeX / resolution_scaler + bbox[0] #*1.25
        elif(scale==2):
            grid_rangeX = grid_rangeX * 2 / resolution_scaler + bbox[0] #*1.25
        elif(scale==3):
        ...
        if(scale==1):
            grid_rangeY = grid_rangeY  / resolution_scaler + bbox[2] #*1.25
        elif(scale==2):
            grid_rangeY = grid_rangeY * 2 / resolution_scaler + bbox[2] #*1.25
        elif(scale==3):
        ...
        if view== 'view1':
            view_ic = np.load('coords_correspondence/projection_forth/view1_new_{}.npz'.format(scale))
        if view== 'view2':
            view_ic = np.load('coords_correspondence/projection_forth/view2_new_{}.npz'.format(scale))
        if view== 'view3':
            view_ic = np.load('coords_correspondence/projection_forth/view3_new_{}.npz'.format(scale))

        
        
        #view_ic = self.World2Image(view, wldcoords)
        view_ic = view_ic['arr_0']
    ...
    def proj_splat(self,  inputs):
        with tf.compat.v1.variable_scope('ProjSplat'):
            ...
            with tf.name_scope('BilinearInterp'):
                ...
                im_x0_f, im_x1_f = tf.cast(im_x0,tf.float32), tf.cast(im_x1,tf.float32)
                im_y0_f, im_y1_f = tf.cast(im_y0,tf.float32), tf.cast(im_y1,tf.float32)
